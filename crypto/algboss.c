...
static int cryptomgr_schedule_probe(struct crypto_larval *larval)
{
	...
	thread = kthread_run_perf_critical(cpu_perf_mask, cryptomgr_probe, param, "cryptomgr_probe");
	if (IS_ERR(thread))
		goto err_put_larval;
...
static int cryptomgr_schedule_test(struct crypto_alg *alg)
{
	...
	thread = kthread_run_perf_critical(cpu_lp_mask, cryptomgr_test, param, "cryptomgr_test");
	if (IS_ERR(thread))
		goto err_free_param;
