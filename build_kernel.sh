...
run_with_timer make ARCH=arm64 -j4 exynos7885-a30s_oneui_defconfig 2>&1 | tee log_defconfig.log

run_with_timer echo "Make kernel..."
run_with_timer make ARCH=arm64 -j4 2>&1 | tee log_kernel.log
...
if [ -s "arch/arm64/boot/Image" ]; then
    ...
    cd AnyKernel3/ && zip -r9 "A30s ${KERNEL_VERSION}.zip" "${FILES[@]}" && mv "A30s ${KERNEL_VERSION}.zip" ../ && cd ..
